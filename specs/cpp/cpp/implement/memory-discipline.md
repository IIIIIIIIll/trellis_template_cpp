---
description: RAII practice, pass-by rules, lifetime and invalidation pitfalls
paths: [**/*.cpp, **/*.cc, **/*.cxx, **/*.hpp, **/*.hh, **/*.h, **/*.inl, **/*.ipp]
---

# Memory Discipline

> RAII practice, pass-by rules, and the lifetime and invalidation pitfalls that decide whether memory bugs reach sanitizers or production.

---

## Overview

Memory safety here is a daily practice, not an architecture diagram: every resource enters a RAII wrapper on the acquisition line, arguments travel by a fixed pass-by table, and any iterator or reference held across a container mutation is checked against the invalidation table first. This guide is the implementation half of ownership — where wrappers land, how parameters travel, and the dangling-view, invalidation, cycle, and arena traps that ASan and LeakSanitizer exist to catch. The design half — which owner a type declares and when a signature takes a smart pointer — lives in the companion Ownership Design guide. Baseline is C++14; C++17/20 upgrades are marked inline where they change the recommendation.
...
The copy-cost side of these same decisions lives in [Functions and Interfaces](../design/functions-and-interfaces.md) under Value versus Const Reference (`F.15`, `F.16`) — its table prices copies, the one below fixes parameter kinds: companions, not rivals.
...
- **MEM-30.** The optional-result row covers *value* results only. A finder that locates an existing object inside storage the caller already owns returns its position as a nullable `T*` (`F.42`, [Functions and Interfaces](../design/functions-and-interfaces.md)) — not a sentinel, and not an `optional<T>` that copies the object out of place.
...
Before merging memory-discipline code, confirm:
